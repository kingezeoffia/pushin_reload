...
## Grace Period API (EXPIRED State)

**IMPORTANT**: The EXPIRED state requires a dedicated API to expose grace period remaining time.

### Why `getGracePeriodRemaining()` Exists

The controller tracks the grace period internally via:
- `_expiredAt`: The timestamp when EXPIRED state was entered
- `_gracePeriodSeconds`: Configurable duration (default 5 seconds)

**Why NOT `getUnlockTimeRemaining()`?**
- `getUnlockTimeRemaining()` returns the unlock session time remaining
- When transitioning to EXPIRED, the unlock session is cleared (or already at 0)
- Using it for grace period display would always show 0, making the countdown incorrect

**Correct API**:
```dart
// ✅ CORRECT: Exposes grace period countdown
int getGracePeriodRemaining(DateTime now);
```

**Usage**:
```dart
// In EXPIRED state UI mapping
gracePeriodRemaining: controller.getGracePeriodRemaining(now)
```

**Contract Compliance**:
- Time is injected via `DateTime now` parameter (no time leaks)
- No side effects or state mutations
- Returns 0 if not in EXPIRED state or grace period elapsed
      ...
      case PushinState.expired:
        if (blockedTargets.isNotEmpty) {
          return HomeUIState.expired(
            blockedTargets: blockedTargets,
            gracePeriodRemaining: _controller.getGracePeriodRemaining(_currentTime), // Correct grace period API
            canStartWorkout: true,
          );
        }
        break;
    ...
    private static func computeUIState(controller: PushinController, now: Date) -> HomeUIState {
        ...
        case .expired:
            if !blockedTargets.isEmpty {
                return .expired(
                    blockedTargets: blockedTargets,
                    gracePeriodRemaining: controller.getGracePeriodRemaining(now), // Correct grace period API
                    canStartWorkout: true
                )
            }
