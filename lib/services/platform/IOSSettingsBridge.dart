...
class IOSSettingsBridge {
  static const MethodChannel _iosChannel = MethodChannel('com.pushin.iossettings');
  static const MethodChannel _androidChannel = MethodChannel('com.pushin.blockingservice');
  ...
  Future<void> setEmergencyUnlockEnabled(bool enabled) async {
    ...
    try {
      if (Platform.isIOS) {
        await _iosChannel.invokeMethod('setEmergencyUnlockEnabled', {'enabled': enabled});
      } else if (Platform.isAndroid) {
        await _androidChannel.invokeMethod('setEmergencyUnlockEnabled', {'enabled': enabled});
      }
    } catch (e) {
  ...
  Future<void> setEmergencyUnlockMinutes(int minutes) async {
    ...
    try {
      if (Platform.isIOS) {
        await _iosChannel.invokeMethod('setEmergencyUnlockMinutes', {'minutes': minutes});
        print('✅ Saved emergency unlock minutes to iOS: $minutes');
      } else if (Platform.isAndroid) {
        await _androidChannel.invokeMethod('setEmergencyUnlockMinutes', {'minutes': minutes});
